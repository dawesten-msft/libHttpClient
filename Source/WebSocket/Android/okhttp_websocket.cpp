...
)
{
    ...
    auto impl = std::dynamic_pointer_cast<okhttp_websocket_impl>(websocket->impl);
    if (!impl)
    {
        websocket->impl = (impl = std::make_shared<okhttp_websocket_impl>(env->GetJavaVm(), env->GetWebSocketClass(), websocket));
    }
